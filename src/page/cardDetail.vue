  ...
  <div class="cardDetail">
    <div class="f-person">
      ...
      <div class="p-name">
        ...
        <p class="start s-top">
          打卡始于
          <span class="p-ye">{{pushStart}}</span>,持续打卡于
          <span class="p-ye">{{pushEnd}}</span>,
        </p>
        <p class="start">
          日精进打卡第
          <span class="p-ye">{{time}}</span>天,共打卡
          <span class="p-ye">{{total}}</span>次,缺卡
          <span class="p-ye">{{que}}</span>次
        </p>
      </div>
    ...
    <div class="f-content">
      ...
      <p class="c-name">
        <span class="fc-n">{{item.user.name}}</span>
        <span v-if="item.user.company && item.user.company!=''">-{{item.user.company}}</span>
      </p>
      <div class="c-content">
        <p class="c-time">
          <span>{{item.pushd}}</span>
          打卡第{{item.pushCount.putCount}}次
        </p>
        <div class="c-text">
          <!-- 知 学习 -->
          <p class="tit">【知～学习】</p>
          <div v-for="(el,bs) in item.books" :key="bs">
            <p
              class="c-color"
              v-if="el.readCount!=0"
            >《{{el.title}}》{{el.readCount}}遍 共{{el.readTotal}}遍</p>
          ...
          <!-- 行 实践 -->
          <template
            v-if="item.practice!=undefined&&(item.practice.character!=''||item.practice.family!=''||item.practice.work!='')"
          >
            <p class="tit">【行～实践】</p>
          ...
          <p
            class="c-color"
          >发愿从{{item.pushCount.bonaStart}}起{{item.pushCount.years}}年内{{item.pushCount.bonaCount*item.pushCount.days}}善事。今日{{item.pushCount.bonaDays}}善，累计{{item.pushCount.bonaTotal}}善</p>
          <!-- 省 觉悟 -->
